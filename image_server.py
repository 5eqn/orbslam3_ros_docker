...
class ImageTrackingDataServer:
    def __init__(self, port=51121):
        ...
        # 等待ROS服务就绪
        rospy.loginfo("=== [Server Init] Waiting for orb_slam3/get_tracking_data service... ===")
        rospy.wait_for_service('orb_slam3/get_tracking_data')
        self.get_tracking_data_srv = rospy.ServiceProxy('orb_slam3/get_tracking_data', GetTrackingData)
        ...
        print("=== [Server Init] Byte stream server starting on 0.0.0.0:{} ===".format(port))
    ...
    def get_ros_tracking_data(self):
        ...
        rospy.loginfo("=== [ROS Service] Start calling orb_slam3_ros/get_tracking_data ===")
        try:
            # 调用ROS服务（无请求参数）
            start_time = time.time()
            response = self.get_tracking_data_srv(GetTrackingDataRequest())
            cost_time = (time.time() - start_time) * 1000
            rospy.loginfo("=== [ROS Service] Call success, cost {:.2f} ms ===".format(cost_time))
            
            # 调试：打印ROS服务返回数据的关键信息
            if response:
                rospy.loginfo("=== [ROS Service] Response info: ===")
                rospy.loginfo("  - Intrinsics: fx={:.2f}, fy={:.2f}, cx={:.2f}, cy={:.2f}".format(
                    response.fx, response.fy, response.cx, response.cy
                ))
                rospy.loginfo("  - Success: {}".format(response.success))
                rospy.loginfo("  - Camera pose valid: {}".format(bool(response.camera_pose)))
                rospy.loginfo("  - Camera point cloud data len: {}".format(len(response.tracked_points_camera.data)))
                rospy.loginfo("  - World point cloud data len: {}".format(len(response.tracked_points_world.data)))
                rospy.loginfo("  - Image valid: {}".format(not response.current_image.data == ''))
            return response
    ...
    def pack_data_to_bytes(self, tracking_data):
        ...
        # 1. 封装内参（4个float64，每个8字节，共32字节）
        fx = tracking_data.fx
        fy = tracking_data.fy
        cx = tracking_data.cx
        cy = tracking_data.cy
        intrinsics_bytes = struct.pack('>dddd', fx, fy, cx, cy)
        byte_buffer.append(intrinsics_bytes)
        total_pack_size += len(intrinsics_bytes)
        rospy.loginfo("=== [Byte Pack] 1. Intrinsics packed: 32 bytes ===")
        
        # 2. 封装服务状态（bool，转换为uint8，1字节）
        ...
        rospy.loginfo("=== [Byte Pack] 2. Success packed: 1 byte ===")
        
        # 3. 封装相机位姿（7个float64，共56字节；空位姿填充0）
        ...
        try:
            ...
            rospy.logwarn("=== [Byte Pack] 3. Pose invalid, fill default value ===")
        ...
        rospy.loginfo("=== [Byte Pack] 3. Pose packed: 56 bytes ===")
        
        # 4. 封装相机坐标点云（tracked_points_camera）
        ...
        rospy.loginfo("=== [Byte Pack] 4. Camera point cloud packed: {} (len) + {} (data) = {} bytes ===".format(
            len(pc_camera_len_bytes), pc_camera_len, len(pc_camera_len_bytes) + pc_camera_len
        ))
        
        # 5. 封装世界坐标点云（tracked_points_world）
        ...
        rospy.loginfo("=== [Byte Pack] 5. World point cloud packed: {} (len) + {} (data) = {} bytes ===".format(
            len(pc_world_len_bytes), pc_world_len, len(pc_world_len_bytes) + pc_world_len
        ))
        
        # 6. 封装图像数据（先jpg长度，后jpg字节数据）
        ...
        try:
            ...
            rospy.logwarn("=== [Byte Pack] 6. Image encode failed: %s ===", e)
        ...
        rospy.loginfo("=== [Byte Pack] 6. Image packed: {} (len) + {} (data) = {} bytes ===".format(
            len(img_len_bytes), img_len, len(img_len_bytes) + img_len
        ))
        
        # 拼接所有字节数据，返回完整字节流
        final_byte_stream = b''.join(byte_buffer)
        rospy.loginfo("=== [Byte Pack] Total byte stream size: {} bytes ===".format(total_pack_size))
    ...
    def handle_client(self, conn, addr):
        ...
        try:
            ...
            rospy.loginfo("=== [Client {}] Waiting for client request... ===".format(addr[0]))
            request = conn.recv(1024)  # 改为接收任意长度，而非等待满1024字节
            
            if not request:
                rospy.logwarn("=== [Client {}] No valid request received, closing connection ===".format(addr[0]))
                conn.sendall(struct.pack('>I', 0))  # 发送空数据标记
                return
            
            rospy.loginfo("=== [Client {}] Received request (size: {} bytes): {}".format(
                addr[0], len(request), request[:50] if len(request) > 50 else request
            ))
            
            # 1. 获取ROS服务数据（含双点云）
            rospy.loginfo("=== [Client {}] Start fetching ROS service data ===".format(addr[0]))
            tracking_data = self.get_ros_tracking_data()
            
            # 2. 封装为字节流（含双点云）
            rospy.loginfo("=== [Client {}] Start packing byte stream ===".format(addr[0]))
            byte_stream = self.pack_data_to_bytes(tracking_data)
            
            # 3. 发送字节流（无额外协议，纯数据传输）
            if byte_stream:
                rospy.loginfo("=== [Client {}] Sending byte stream (size: {} bytes) ===".format(
                ...
                for i in range(0, len(byte_stream), chunk_size):
                    ...
                    conn.sendall(chunk)
                rospy.loginfo("=== [Client {}] Byte stream sent completely ===".format(addr[0]))
            else:
                rospy.logwarn("=== [Client {}] Empty byte stream, sending null marker ===".format(addr[0]))
                conn.sendall(struct.pack('>I', 0))
        
        except socket.timeout:
            rospy.logerr("=== [Client {}] Request recv timeout (5s), no data received ===".format(addr[0]))
        except Exception as e:
